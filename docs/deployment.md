...
4. **Neon은 Launch 티어**를 쓴다. 현황 화면 폴링이 DB를 깨워두므로 Free 티어의 100 CU-시간/월 한도를 며칠 만에 소진한다. **2026-08-27 실제로 이 한도를 소진해 운영 장애가 났다** (Postgres 53000, 로그인·전 화면 실패 → Launch 전환으로 즉시 복구).
   - **Autosuspend는 켜 둔다(끄지 않는다).** Neon은 질의 횟수가 아니라 *컴퓨트가 깨어 있는 시간*으로 과금하므로($0.106/CU-시간, 최소 0.25 CU), 마감 후 재우는 것이 유일하게 요금이 줄어드는 수단이다. 이를 위해 현황 화면은 영업시간(11:00~03:00) 밖이거나 화면이 보이지 않으면 폴링을 멈춘다 (`isWithinLiveRefreshWindow`).
   - 폴링 **주기를 늘리는 것(15초→30초)만으로는 요금이 줄지 않는다.** 절전 대기시간(기본 5분, 최소 60초)보다 짧은 간격이면 DB는 계속 깨어 있다.
...
> ⚠️ **현재 운영 DB(`erp-vietnam-aesthetic-sg`)에는 이미 실데이터가 들어있다** (2026-06-21에 기존 us-east-1 DB에서 `pg_dump`/`pg_restore`로 계정·마스터·운영 데이터 일체를 이전함). 따라서 **재시드하지 말 것** — `seed-dev-accounts.ts`는 개발용 기본 비밀번호로 계정을 upsert하므로 운영 계정 비번을 덮어쓴다(보안 위험).
...
- [x] `/sign-in` 실제 로그인 동작 (이전된 운영 계정으로) (2026-08-27 확인)
- [ ] 콜 원장에서 행 저장이 빠른지 (앱·DB가 같은 싱가포르 리전이면 즉각 반응)
- [ ] 객실/TV 현황이 30초마다 갱신되는지 (영업시간 외에는 "영업 시간 외 · 자동 갱신 중지" 표시가 뜨고 멈추는지)
- [x] 월마감 미리보기/대시보드가 뜨는지 (2026-08-27 확인)
- [x] (운영자) Neon 요금제를 **Launch**로 올렸는지 (2026-08-27 전환. Autosuspend는 의도적으로 켜 둔다 — 위 핵심 원칙 4 참고)
- [ ] (운영자) Vercel 플랜이 **Pro**인지 — **2026-08-27 기준 아직 `hobby`.** 상업 운영은 Hobby 약관 위반이고, 런타임 로그도 1시간치만 남아 장애 조사가 어렵다.
