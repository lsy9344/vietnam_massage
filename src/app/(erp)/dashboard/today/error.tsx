...
export default function TodayDashboardError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <main className="min-h-screen px-4 py-6 lg:px-8 lg:py-7">
      <PageHeader eyebrow={t("dashboard.eyebrow")} title={t("dashboard.today.title")} />
      <section className="border border-danger bg-surface px-4 py-6" role="alert">
        <h2 className="text-base font-semibold text-foreground">{t("dashboard.today.error.title")}</h2>
        <p className="mt-2 max-w-2xl text-sm text-muted">{t("dashboard.today.error.description")}</p>
