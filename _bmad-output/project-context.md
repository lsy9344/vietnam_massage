...
- 매출 반영(선결제, REQ-007)은 "결제수단 선택 시점"을 단일 기준으로 한다. 결제수단이 선택된 콜은 `방문완료` 이전(예약·사용중)이라도 매출/결제수단별 집계에 포함하되, `취소`/`노쇼`는 결제수단이 있어도 매출에서 즉시 제외한다. 결제수단이 없는 `사용중`은 매출에 포함하지 않는다.
- 수당/귀케어풀/콜인정은 여전히 `방문완료` calculated 콜만을 기준으로 하며, 비완료 콜의 선결제 매출이 정산 계산으로 새지 않는다.
...
- Story 7.2 기준 핵심 계산 대조는 `tests/fixtures/migration-calculation-comparison.ts`의 공용 fixture source of truth와 `tests/fixtures/migration-calculation-prisma.ts` read-only memory Prisma adapter를 사용한다. Migration module에 새 계산 엔진을 만들지 말고 `listCompletedServiceCallCalculationsForDate()`, `getDailyCallLedgerSummary()`, `listRoomStatuses()`의 `RoomStatusDto`, therapist/earcare/operations settlement services, `listMonthlyClosingPreview()` 같은 기존 domain service를 재사용해야 한다.
- Story 7.2 fixture는 `방문완료`, `예약`, `사용중`, `청소중`, `노쇼`, `취소`, 고정 할인 `100000`, `THERAPIST_1`, `THERAPIST_2`, D코스 마사지사2 필수 검증, 운영팀 30/40/50 일일 기준과 1000/1100/1200/1300/1400/1500 월 기준, 귀케어 0명, 8시간 만근 인정, 20일 만근수당, 40콜 갯수왕 기준을 포함해야 한다.
- Story 7.2 계산 불일치 report shape는 `area`, `fixtureId`, `expected`, `actual`, `sourceReference`, `relatedRequirement`, `message`로 고정한다. Excel 셀 좌표와 workbook range는 source evidence 문자열로만 보존하고 테스트 로직에서 parse하거나 구현 기준으로 쓰지 않는다. 모든 reference key는 stable ID를 사용한다.
- Story 7.3 기준 이관 검증 report DTO source는 `src/modules/migration/migration-verification-report.ts`다. DTO는 `summary`, `sheetRows`, `calculationRows`, `openIssueRows`, `filters`, `generatedAt`을 제공하고 Story 7.1 `SHEET_FEATURE_MAPPINGS`와 Story 7.2 `MigrationMismatchReport`를 소비해야 한다.
- Story 7.3 기준 persisted QA tracking은 Prisma `MigrationVerificationIssue`와 `MigrationVerificationIssueHistory`가 소유한다. 일반 audit log는 계산 근거나 QA 추적 저장소로 재사용하지 않는다. deterministic key는 `sheet:목록`, `calculation:calls.payment:call-complete-a-discount`, `fr:FR-37` 같은 형태를 사용한다.
- Story 7.3 tracking status values는 정확히 `미확인`, `수정중`, `재검증 필요`, `통과` 네 값이다. 상태 변경은 administrator-only `migration:write` 경계와 service-level validation을 거치고 history에는 이전 상태, 새 상태, 담당자 메모, 변경자 account id, 변경 시각을 남긴다.
- Story 7.3 기준 hidden `목록` 100% gate는 visible sheet와 동일 가중치다. 숨김 목록 또는 시트별 계산 항목이 누락되면 기능 보존율을 100%로 표시하면 안 된다.
- Story 7.3 기준 no runtime Excel parsing 원칙을 유지한다. report service, route, action은 `sheet.xlsx`, Excel formula, cell coordinate, workbook range를 런타임에 parse하지 않고 source evidence 문자열로만 표시한다.
- Story 7.3 기준 `/masters/sheet-mapping`은 administrator와 `read_only_viewer`의 QA/reference route다. read-only viewer exact access는 `/masters/sheet-mapping`만 허용하고 `/masters` prefix를 열지 않는다. `read_only_viewer`와 `waiter`는 migration issue mutation affordance를 보지 못해야 한다.
...
- Story 3.4 기준 `/tv`는 조회 전용/읽기 전용 downstream 화면이다. `requireRouteAccess("/tv")`를 유지하고 administrator/counter/read_only_viewer만 접근한다(요구사항 9.1 권장 권한의 카운터 "TV 화면 조회"). waiter/settlement_manager 권한은 넓히지 않고, counter에게도 조회 외 권한은 추가하지 않는다.
